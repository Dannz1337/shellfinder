       ...
       {bcolors.BLEU}
          ...
          [Warning] I am not responsible for the way you will use this program [Warning]
                ...
                | (___ | |__   ___| | | | |__   _ _ __   __| | ___ _ __ 
                 \___ \| '_ \ / _ \ | | |  __| | | '_ \ / _` |/ _ \ '__| 
                 ____) | | | |  __/ | | | |    | | | | | (_| |  __/ |   
                |_____/|_| |_|\___|_|_| |_|    |_|_| |_|\__,_|\___|_|   {bcolors.RESET}
...
class ProxyRotator:
    ...
    def load_proxies_from_file(self, filename):
        """Load proxies from file with format ip:port per line"""
        try:
            ...
            print(f"{bcolors.GREEN}[+] Loaded {len(self.proxies)} proxies from {filename}{bcolors.RESET}")
        except FileNotFoundError:
            print(f"{bcolors.RED}[!] Proxy file '{filename}' not found.{bcolors.RESET}")
        except Exception as e:
            print(f"{bcolors.RED}[!] Error loading proxies: {e}{bcolors.RESET}")
    
    def add_proxy(self, proxy_str):
        """Add a single proxy in format ip:port"""
    ...
    def get_next_proxy(self):
        ...
        return proxy
    # Di dalam class ProxyRotator, perbaiki fungsi get_proxy_dict
def get_proxy_dict(self, proxy_str=None):
    """Convert proxy string to requests proxy dict with authentication support"""
    if not proxy_str and not self.proxies:
        return None
    ...
    try:
        # Jika proxy sudah mengandung http:// atau https://
        if proxy_str.startswith('http://') or proxy_str.startswith('https://'):
            # Jika mengandung @ (autentikasi), biarkan sebagaimana adanya
            if '@' in proxy_str:
                return {
                    'http': proxy_str,
                    'https': proxy_str.replace('http://', 'https://') if 'http://' in proxy_str else proxy_str
                }
            else:
                return {
                    'http': proxy_str,
            ...
            if '@' in proxy_str:
                # Sudah ada autentikasi di string
                return {
                    'http': f'http://{proxy_str}',
                    'https': f'http://{proxy_str}'  # Bisa juga ganti ke https://
                }
            else:
        ...
        print(f"{bcolors.RED}[!] Error parsing proxy {proxy_str}: {e}{bcolors.RESET}")
        return None
    # Perbaiki fungsi setup_proxy_rotator
def setup_proxy_rotator():
    print(f"\n{bcolors.GREEN}[+] {bcolors.BLEU}Proxy Configuration:")
    print(f"{bcolors.YELLOW}\n1. Use proxy rotator (load proxies from file)")
    print(f"{bcolors.YELLOW}2. Use single proxy")
    print(f"{bcolors.YELLOW}3. No proxy (direct connection)")
    
    choice = input(f"{bcolors.GREEN}\n[+] Your choice {bcolors.BLEU}(1, 2, or 3): ")
    
    proxy_rotator = ProxyRotator()
    
    if choice == "1":
        print(f"\n{bcolors.YELLOW}[+] Proxy file format options:")
        print(f"{bcolors.BLEU}   a) ip:port (without auth)")
        print(f"{bcolors.BLEU}   b) username:password@ip:port (with auth)")
        print(f"{bcolors.BLEU}   c) http://ip:port")
        print(f"{bcolors.BLEU}   d) http://username:password@ip:port")
        
        proxy_file = input(f"{bcolors.YELLOW}\n[+] Enter proxy file path: ").strip()
        proxy_rotator.load_proxies_from_file(proxy_file)
        if proxy_rotator.is_empty():
            print(f"{bcolors.RED}[!] No proxies loaded. Continuing without proxy.{bcolors.RESET}")
        else:
            print(f"{bcolors.GREEN}[+] Loaded {proxy_rotator.get_count()} proxies.{bcolors.RESET}")
            
    elif choice == "2":
        print(f"\n{bcolors.YELLOW}[+] Proxy format options:")
        print(f"{bcolors.BLEU}   a) ip:port (without auth)")
        print(f"{bcolors.BLEU}   b) username:password@ip:port (with auth)")
        print(f"{bcolors.BLEU}   c) http://ip:port")
        print(f"{bcolors.BLEU}   d) http://username:password@ip:port")
        
        proxy_str = input(f"{bcolors.YELLOW}\n[+] Enter proxy: ").strip()
        
        # Validasi format proxy
        if ':' not in proxy_str:
            print(f"{bcolors.RED}[!] Invalid proxy format. Should contain ':'{bcolors.RESET}")
        else:
            proxy_rotator.add_proxy(proxy_str)
            print(f"{bcolors.GREEN}[+] Single proxy configured.{bcolors.RESET}")
            
    elif choice == "3":
        print(f"{bcolors.GREEN}[+] No proxy will be used.{bcolors.RESET}")
    else:
        print(f"{bcolors.RED}[!] Invalid choice. Continuing without proxy.{bcolors.RESET}")
...
def send_telegram_message(token, chat_id, message):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"{bcolors.GREEN}\n[+] Message sent to Telegram successfully.{bcolors.RESET}")
        else:
            print(f"{bcolors.RED}\n[!] Failed to send message. Error: {response.text}{bcolors.RESET}")
    except requests.exceptions.RequestException as e:
        print(f"{bcolors.RED}[!] Error sending message: {e}{bcolors.RESET}")

# Fonction pour demander à l'utilisateur s'il veut envoyer les résultats par Telegram
def get_telegram_info():
    send_telegram = input(f"{bcolors.GREEN}[+] {bcolors.YELLOW}Do you want to send the results to Telegram? {bcolors.BLEU}(yes/no): ").strip().lower()

    if send_telegram == "yes":
        token = input(f"{bcolors.YELLOW}[+] Enter your Telegram bot token: ").strip()
        chat_id = input(f"{bcolors.YELLOW}[+] Enter your chat ID: ").strip()
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"{bcolors.GREEN}\n[+] Token is valid.{bcolors.RESET}")
                return token, chat_id
            else:
                print(f"{bcolors.RED}\n[!] Invalid token or chat ID. Please try again.{bcolors.RESET}")
                return None, None
        except requests.exceptions.RequestException as e:
            print(f"{bcolors.RED}\n[!] Error connecting to Telegram API: {e}{bcolors.RESET}")
            return None, None
    else:
        print(f"{bcolors.RED}\n[+] Continuing without Telegram.{bcolors.RESET}")
        return None, None
...
def get_shell_list():
    print(f"\n{bcolors.BLEU}[+] Choose how to provide the shell list:\n")
    print(f"{bcolors.YELLOW}1. Use the default list of shells.")
    print(f"{bcolors.YELLOW}2. Provide a file containing shells (one per line).")
    
    choice = input(f"{bcolors.GREEN}\n[+] Your choice {bcolors.BLEU}(1 or 2): ")
    
    shell_list = []

    if choice == "1":
        print(f"{bcolors.GREEN}\n[+] Using default shell list.{bcolors.RESET}")
        shell_list = default_shells
    elif choice == "2":
        filename = input(f"{bcolors.YELLOW}\n[+] Enter the filename with the list of shells: ").strip()
        try:
            with open(filename, 'r') as file:
                shell_list = [line.strip() for line in file.readlines() if line.strip()]
            print(f"{bcolors.GREEN}[+] Loaded {len(shell_list)} shells from {filename}.{bcolors.RESET}")
        except FileNotFoundError:
            print(f"{bcolors.RED}[!] File '{filename}' not found.{bcolors.RESET}")
            return []
    else:
        print(f"{bcolors.RED}[!] Invalid choice.{bcolors.RESET}")
        return []
...
def get_website_list():
    print(f"\n{bcolors.GREEN}[+] {bcolors.BLEU}Choose how to provide the websites for shell search:\n")
    print(f"{bcolors.YELLOW}1. Enter a list of URLs manually (separated by new lines).")
    print(f"{bcolors.YELLOW}2. Provide a file containing URLs (one per line).")
    ...
    if choice == "1":
        ...
        while True:
            website = input(f"\n{bcolors.BLEU}[+] Enter URL: ")
            if website.lower() == 'done':
                break
            if validate_url(website):
                website_list.append(website)
            else:
                print(f"{bcolors.RED}\n[!] Invalid URL format. Please try again.{bcolors.RESET}")
    elif choice == "2":
        filename = input(f"{bcolors.GREEN}\n[+] Enter the filename with the list of URLs: ")
        try:
            with open(filename, 'r') as file:
                website_list = [line.strip() for line in file.readlines() if line.strip()]
            print(f"{bcolors.GREEN}\n[+] Loaded {len(website_list)} URLs from {filename}.{bcolors.RESET}")
        except FileNotFoundError:
            print(f"{bcolors.RED}\n[!] File '{filename}' not found.{bcolors.RESET}")
            return []
    else:
        print(f"{bcolors.RED}\n[!] Invalid choice.{bcolors.RESET}")
        return []

    return website_list

# Fonction pour configurer proxy rotator
def setup_proxy_rotator():
    print(f"\n{bcolors.GREEN}[+] {bcolors.BLEU}Proxy Configuration:")
    print(f"{bcolors.YELLOW}\n1. Use proxy rotator (load proxies from file)")
    print(f"{bcolors.YELLOW}2. Use single proxy")
    ...
    if choice == "1":
        proxy_file = input(f"{bcolors.YELLOW}\n[+] Enter proxy file path (format: ip:port per line): ").strip()
        proxy_rotator.load_proxies_from_file(proxy_file)
        if proxy_rotator.is_empty():
            print(f"{bcolors.RED}[!] No proxies loaded. Continuing without proxy.{bcolors.RESET}")
    elif choice == "2":
        proxy_str = input(f"{bcolors.YELLOW}\n[+] Enter proxy (format: ip:port or http://ip:port): ").strip()
        proxy_rotator.add_proxy(proxy_str)
        print(f"{bcolors.GREEN}[+] Single proxy configured.{bcolors.RESET}")
    elif choice == "3":
        print(f"{bcolors.GREEN}[+] No proxy will be used.{bcolors.RESET}")
...
def find_shells():
    ...
    # Setup proxy rotator
    proxy_rotator = setup_proxy_rotator()
    
    websites = get_website_list()
    
    if not websites:
        print(f"{bcolors.RED}\n[!] No websites provided. Exiting...{bcolors.RESET}")
        return

    shells = get_shell_list()
    
    if not shells:
        print(f"{bcolors.RED}\n[!] No shells provided. Exiting...{bcolors.RESET}")
        return
    ...
    with requests.Session() as session:
        ...
        for website in websites:
            ...
            for shell in shells:
                ...
                proxy_dict = None
                if not proxy_rotator.is_empty():
                    proxy_dict = proxy_rotator.get_proxy_dict()
                    current_proxy = proxy_rotator.proxies[proxy_rotator.current_index - 1]
                    print(f"{bcolors.BLEU}[+] Using proxy: {current_proxy}{bcolors.RESET}")
                
                try:
                    response = session.get(host, timeout=10, proxies=proxy_dict)
                    
                    if response.status_code == 200:
                        print(f'{bcolors.GREEN}[+] Shell found: {host}{bcolors.RESET} {bcolors.BLEU}[+] Code: 200{bcolors.RESET}')
                        foundshells.append(host)
                    elif response.status_code == 403:
                        print(f'{bcolors.RED}[-] Forbidden: {host} {bcolors.YELLOW}[+] Code: 403{bcolors.RESET}')
                    elif response.status_code == 404:
                        print(f'{bcolors.RED}[-] Not Found: {host} {bcolors.YELLOW}[+] Code: 404{bcolors.RESET}')
                    else:
                        print(f'{bcolors.RED}[-] HTTP {response.status_code}: {host}{bcolors.RESET}')
                        
                except requests.exceptions.Timeout:
                    print(f"{bcolors.RED}[!] Timeout error with {host}{bcolors.RESET}")
                except requests.exceptions.ProxyError:
                    print(f"{bcolors.RED}[!] Proxy error with {host}{bcolors.RESET}")
                    # Try without proxy on proxy error
                    try:
                        response = session.get(host, timeout=5)
                        if response.status_code == 200:
                            print(f'{bcolors.GREEN}[+] Shell found (direct): {host}{bcolors.RESET} {bcolors.BLEU}[+] Code: 200{bcolors.RESET}')
                            foundshells.append(host)
                    except:
                        pass
                except requests.exceptions.RequestException as e:
                    print(f"{bcolors.RED}[!] Error with {host}: {e}{bcolors.RESET}")

                # Délai entre les requêtes pour éviter le rate limiting
                time.sleep(0.5)
                
                # Afficher la progression toutes les 10 requêtes
                if request_count % 10 == 0:
                    print(f"{bcolors.BLEU}[+] Requests made: {request_count}, Shells found: {len(foundshells)}{bcolors.RESET}")

    if foundshells:
        fpth = os.getcwd()
        fpth2 = os.path.join(fpth, 'found.txt')

        # Écrire les shells trouvés dans le fichier
        with open(fpth2, 'w') as fob:
            for shell in foundshells:
                fob.write(shell + '\n')

        print(f'{bcolors.GREEN}\n[+] Found {len(foundshells)} shells saved in: {fpth2}{bcolors.RESET}')
        
        # Envoi sur Telegram si demandé
        if token and chat_id:
            message = f"Found {len(foundshells)} shells:\n" + "\n".join(foundshells)
            send_telegram_message(token, chat_id, message)
    else:
        print(f'{bcolors.RED}\n[+] No shells found.{bcolors.RESET}')

    print(f"{bcolors.BLEU}\n[+] Total requests made: {request_count}{bcolors.RESET}")
    print(f"{bcolors.BLEU}[+] Proxies used: {proxy_rotator.get_count()}{bcolors.RESET}")
    
    input(f"{bcolors.BLEU}\n[+] Press enter to exit...{bcolors.RESET}")
